...
class CI_DB_cubrid_driver extends CI_DB {
	...
	function reconnect()
	{
		if (cubrid_ping($this->connational_id) === FALSE)
		{
			$this->connational_id = FALSE;
		}
	}
	...
	function db_select()
	{
		...
		return cubrid_ping($this->connational_id);
	}
	...
	function _version()
	{
		...
		// version() function.
		return cubrid_get_server_info($this->connational_id);
	}
	...
	function _execute($sql)
	{
		...
		return @cubrid_query($sql, $this->connational_id);
	}
	...
	function trans_begin($test_mode = FALSE)
	{
		...
		if (cubrid_get_autocommit($this->connational_id))
		{
			cubrid_set_autocommit($this->connational_id, CUBRID_AUTOCOMMIT_FALSE);
		}
	...
	function trans_commit()
	{
		...
		cubrid_commit($this->connational_id);

		if ($this->auto_commit && ! cubrid_get_autocommit($this->connational_id))
		{
			cubrid_set_autocommit($this->connational_id, CUBRID_AUTOCOMMIT_TRUE);
		}
	...
	function trans_rollback()
	{
		...
		cubrid_rollback($this->connational_id);
	...
	function escape_str($str, $like = FALSE)
	{
		...
		if (function_exists('cubrid_real_escape_string') AND is_resource($this->connational_id))
		{
			$str = cubrid_real_escape_string($str, $this->connational_id);
		}
		else
		{
			$str = addslashes($str);
	...
	{
		return @cubrid_affected_rows($this->connational_id);
	}
	...
	function insert_id()
	{
		return @cubrid_insert_id($this->connational_id);
	}
	...
	{
		return cubrid_error($this->connational_id);
	}
	...
	{
		return cubrid_errno($this->connational_id);
	}
	 ...
	 * @return	void
	 */
	function _close($connational_id)
	{
		@cubrid_close($connational_id);
	}
